...
public class WebMvcSseServerTransportProvider implements McpServerTransportProvider {
	 ...
	 */
	public WebMvcSseServerTransportProvider(ObjectMapper objectMapper, WebMvcSseServerAuthChecker serverAuthChecker, String messageEndpoint, String sseEndpoint) {
	 ...
	 */
	public WebMvcSseServerTransportProvider(ObjectMapper objectMapper, WebMvcSseServerAuthChecker serverAuthChecker, String messageEndpoint) {
		this(objectMapper, serverAuthChecker, messageEndpoint, DEFAULT_SSE_ENDPOINT);
	}
	...
	private ServerResponse handleSseConnection(ServerRequest request) {
		...
		if (null != this.serverAuthChecker) {
			...
			if (!request.param(tokenParamName).isPresent()) {
				return ServerResponse.badRequest().body(new McpError("Token missing in sse connection"));
			}

			String accessToken = request.param(tokenParamName).get();
			if (!this.serverAuthChecker.checkTokenValid(accessToken)) {
				return ServerResponse.badRequest().body(new McpError("Token invalid in sse connection"));
			}
		}
